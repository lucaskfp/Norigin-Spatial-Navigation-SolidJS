...
# [3.0.0]
## Changed
- **[BREAKING]** Library converted from React to SolidJS. Use `createFocusable` instead of `useFocusable`; `focused` and `hasFocusedChild` are now Solid accessors (call `focused()` / `hasFocusedChild()`). Use `FocusContext.Provider` from the package for focus context. Callback ref: assign `ref={result.ref}` to the focusable element.
- **[BREAKING]** Peer dependency is now `solid-js` (^1.0.0) instead of `react`.
## Removed
- Demo app (`App.tsx`) and related dev server / `index.html` have been removed. This repository is the library core only.
